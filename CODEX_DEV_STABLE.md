...
- Snapshot time: `2026-05-29 19:30:00`
...
- `src\ashare\research_brain\hub\training_engine.py` now derives an execution-aligned label (for example `future_ret_5__entry_lag_1`) from `close` before model fitting, so V5 training metrics use the same next-bar entry convention as the stricter backtest instead of scoring on a same-bar label. Date splits now include an embargo of `label_horizon + execution_lag_bars` trading days, and `train_summary.json` records `time_alignment` plus `overfit_diagnostics`.
