...
def _stage_labels(mode: str) -> list[str]:
    labels: list[str] = []
...
def run_cycle(config_path: Path, mode: str = "full_cycle") -> None:
    """运行单轮研究计划编排。"""
    config = load_config(config_path=config_path)
    project_root = config_path.resolve().parent.parent
    prompt_root = project_root / "prompts"
    stage_labels = _stage_labels(mode=mode)
    log_line(config, f"研究计划: 进入单轮编排，mode={mode} stages={len(stage_labels)}")
    ...
    if mode in {"ingest_only", "extract_only", "gap_only", "plan_only", "bridge_only", "full_cycle"}:
        stage_idx += 1
        t0 = perf_counter()
        log_line(config, f"研究计划: [{stage_idx}/{len(stage_labels)}] 基础表刷新与事件抓取开始")
        ...
        log_line(
            config,
            f"研究计划: [{stage_idx}/{len(stage_labels)}] 公告证据卡摘要 selected={evidence_result.get('selected_items', 0)} "
            f"cards={len(evidence_cards)} path={evidence_result.get('path', '')}",
        )
        log_line(config, f"研究计划: [{stage_idx}/{len(stage_labels)}] 基础表刷新与事件抓取完成 raw_items={len(raw_items)} elapsed={perf_counter() - t0:.1f}s")

    if mode in {"extract_only", "gap_only", "plan_only", "bridge_only", "full_cycle"}:
        stage_idx += 1
        t0 = perf_counter()
        log_line(config, f"研究计划: [{stage_idx}/{len(stage_labels)}] 事件抽取开始 input_raw_items={len(raw_items)}")
        ...
        log_line(
            config,
            f"研究计划: [{stage_idx}/{len(stage_labels)}] 人工复核队列摘要 queue_size={review_result.get('queue_size', 0)} "
            f"path={review_result.get('path', '')}",
        )
        log_line(config, f"研究计划: [{stage_idx}/{len(stage_labels)}] 事件抽取完成 structured_events={len(structured_events)} elapsed={perf_counter() - t0:.1f}s")

    if mode in {"industry_router_only", "plan_only", "bridge_only", "full_cycle"}:
        stage_idx += 1
        t0 = perf_counter()
        log_line(config, f"研究计划: [{stage_idx}/{len(stage_labels)}] 分行业研究骨架开始")
        ...
        log_line(
            config,
            f"研究计划: [{stage_idx}/{len(stage_labels)}] 分行业研究骨架完成 "
    ...
    if mode in {"plan_only", "bridge_only", "full_cycle"}:
        stage_idx += 1
        t0 = perf_counter()
        log_line(config, f"研究计划: [{stage_idx}/{len(stage_labels)}] 市场状态与资金面总阀门开始")
        ...
        log_line(
            config,
            f"研究计划: [{stage_idx}/{len(stage_labels)}] 市场状态与资金面总阀门完成 "
        ...
        log_line(
            config,
            f"研究计划: integrated_thesis refreshed primary={integrated_thesis_payload.get('primary_strategy_key', '')} "
    ...
    if mode in {"gap_only", "plan_only", "bridge_only", "full_cycle"}:
        stage_idx += 1
        t0 = perf_counter()
        log_line(config, f"研究计划: [{stage_idx}/{len(stage_labels)}] 数据清单与缺口分析开始")
        ...
        log_line(
            config,
            f"研究计划: [{stage_idx}/{len(stage_labels)}] 数据清单与缺口分析完成 datasets={len(list(inventory.get('datasets', []) or []))} "
    ...
    if mode in {"plan_only", "bridge_only", "full_cycle"}:
        stage_idx += 1
        t0 = perf_counter()
        log_line(config, f"研究计划: [{stage_idx}/{len(stage_labels)}] 研究证据包与研究计划开始")
        ...
        log_line(config, f"研究计划: 研究证据包已合并公告证据卡 evidence_cards={len(list(context_pack.get('evidence_cards', []) or []))}")
        ...
        log_line(config, f"研究计划: [{stage_idx}/{len(stage_labels)}] 研究证据包与研究计划完成 elapsed={perf_counter() - t0:.1f}s")

    if mode in {"bridge_only", "full_cycle"}:
        stage_idx += 1
        t0 = perf_counter()
        log_line(config, f"研究计划: [{stage_idx}/{len(stage_labels)}] 桥接产物生成开始")
        actions = build_research_actions(brief=research_brief)
        save_bridge_outputs(config=config, actions=actions)
        log_line(
            config,
            f"研究计划: [{stage_idx}/{len(stage_labels)}] 桥接产物生成完成 feature_profiles={len(list(actions.get('candidate_override', {}).get('feature_profiles', []) or []))} "
